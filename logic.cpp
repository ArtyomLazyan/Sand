...
std::vector<std::vector<int>> CLogicEncoder::generateKey()
{
    ...
    size_t count = 3 * m_nColNum * m_nRowNum;
    std::map<std::pair<size_t, size_t>, std::pair<int, short>> mapIndexes;
    size_t unusedVal = 0;
    while(count > 0)
    {
        int i = rand() % m_nRowNum;
        int j = rand() % m_nColNum;
        oLogic.checkField(i, j, unusedVal, false, 1);
        count--;
    }
...
bool CLogicEncoder::getDecodedAndFinished(std::vector<std::vector<int> > &matrix)
{
    ...
    for(size_t j = 0; j < m_nColNum; ++j)
    {
        m_pLogicForDec->checkField(m_nCheckedRow, j, nCount, false, m_arrValuesToCheck[m_nCheckedRow][j]);
    }
    std::map<std::pair<size_t, size_t>, std::pair<int, short>> mapIndexes;
    ...
    for(size_t i = 0; i < m_nRowNum; ++i)
    {
        for(size_t j = 0; j < m_nColNum; ++j)
        {
            matrix[i].push_back(m_arrImage[i][j] - 2);
        }
    }
    m_nCheckedRow++;
